...
class BasicClustersGenerator:
    ...
    def __generate_nodes_positions_for_provided_cluster(
        ...
        if sorted(nodes) == [1, 2, 3, 4, 5, 6, 7, 8]:
            ...
            radius: float = utils_3d.get_radius_for_cluster_with_provided_number_of_nodes(self.__number_of_clusters - 2)
            offset_angle: float = utils_3d.get_offset_angle(self.__number_of_clusters - 2)
            offset_angle_for_current_cluster = utils_3d.get_offset_angle_for_current_cluster(offset_angle, cluster_number)

            positions = {
                8 * cluster_number - 7: (-2, radius + 1, 0),
                8 * cluster_number - 6: (0, radius + 1, 0),
                8 * cluster_number - 5: (2, radius + 1, 0),
                8 * cluster_number - 4: (-1, radius, 0),
                8 * cluster_number - 3: (1, radius, 0),
                8 * cluster_number - 2: (-2, radius - 1, 0),
                8 * cluster_number - 1: (0, radius - 1, 0),
                8 * cluster_number: (2, radius - 1, 0),
            }

            positions = {node: utils_3d.rotate_dot(position, offset_angle_for_current_cluster) for node, position in
                         positions.items()}
    ...
    def __generate_all_clusters_graph(self) -> None:
        ...
        self.__positions: Dict[int, Tuple[float, float, float]] = {}
